...
describe("NFTLocker", () => {
  ...
  describe("NFT tranfers", async () => {
    it("contract does not accept non-configured NFTs", async () => {
      await expectRevert(
        nftA["safeTransferFrom(address,address,uint256)"](
          user1.address,
          locker.target,
          TOKEN_ID
        ),
  ...
  describe("'deposit'", async () => {
    it("adds correct data to 'configuredDeposits'", async () => {
      await nftA.approve(locker.target, TOKEN_ID)
      await locker.deposit(nftA.target, TOKEN_ID, 10)
      const depositData = await locker.deposits(
        user1.address,
        nftA.target,
        TOKEN_ID
      )
    ...
    it("lockForDays must be larger then 0", async () => {
      await nftA.approve(locker.target, TOKEN_ID)
      await expectRevert(locker.deposit(nftA.target, TOKEN_ID, 0), "Invalid")
    })
    ...
    it("transfers NFT token to the contract", async () => {
      ...
      expect(newOwner).to.equal(locker.target)
    })

    it("reverts unless target NFT was approved for transfer", async () => {
      await expectRevert(
        locker.deposit(nftA.target, TOKEN_ID, 10),
    ...
    it("reverts unless target NFT is owner by the account", async () => {
      await expectRevert(
        locker.deposit(nftA.target, TOKEN_ID + 1, 10),
        "caller is not token owner"
      )
    })
    ...
    it("allows deposit for multiple token IDs", async () => {
      ...
      await nftA.approve(locker.target, TOKEN_ID - 1)
      await locker.deposit(nftA.target, TOKEN_ID - 1, 20)
      const depositData2 = await locker.deposits(
        user1.address,
        nftA.target,
        TOKEN_ID - 1
      )
    ...
    it("allows deposit for different NFT types", async () => {
      await nftB.mint()
      await nftB.approve(locker.target, 0)
      await locker.deposit(nftB.target, 0, 15)

      await nftA.approve(locker.target, TOKEN_ID)
      await locker.deposit(nftA.target, TOKEN_ID, 10)

      const depositData = await locker.deposits(user1.address, nftB.target, 0)
      expect(depositData.lockForDays).to.equal(15)
    })

    it("does not allow deposit for the same token IDs", async () => {
      await nftA.approve(locker.target, TOKEN_ID)
      await locker.deposit(nftA.target, 1, 10)

      await expectRevert(
        locker.deposit(nftA.target, 1, 10),
        "already configured"
      )
    })
  })
  ...
  describe("canWithdraw", async () => {
    it("token was not configured it raises an error", async () => {
      await expectRevert(
        locker.canWithdraw(user1.address, nftA.target, TOKEN_ID),
        "not configured"
      )
      ...
      const result = await locker.canWithdraw(
        user1.address,
        nftA.target,
        TOKEN_ID
      )
  ...
  describe("'withdraw'", async () => {
    it("raises an error for not configured tokens", async () => {
      await expectRevert(
        locker.withdraw(nftA.target, TOKEN_ID),
        "not configured"
      )
    })
    ...
    it("returns correct NFT tokens only when withdrawal conditions are correct", async () => {
      ...
      await expect(
        locker.withdraw(nftA.target, TOKEN_ID)
      ).to.changeTokenBalances(nftA, [locker.target, user1.address], [-1, 1])
    ...
    it("does not allow withdrawing by other account", async () => {
      ...
      await expectRevert(
        locker.connect(user2).withdraw(nftA.target, TOKEN_ID),
        "not configured"
      )
    })
    ...
    it("does not affect balances of other users", async () => {
      await nftA.approve(locker.target, TOKEN_ID)
      await nftA.connect(user2).approve(locker.target, TOKEN_ID + 1)

      await locker.deposit(nftA.target, TOKEN_ID, 10)
      await locker.connect(user2).deposit(nftA.target, TOKEN_ID + 1, 20)

      await advanceByDays(12)

      await expectRevert(
        locker.connect(user2).withdraw(nftA.target, TOKEN_ID + 1),
        "cannot withdraw"
      )
      ...
      const depositData1 = await locker.deposits(
        user1.address,
        nftA.target,
        TOKEN_ID
      )
  ...
  describe("'increaseLockForDays'", async () => {
    ...
    it("raises an error for not configured tokens", async () => {
      await expectRevert(
        locker.increaseLockForDays(nftA.target, TOKEN_ID + 1, 25),
        "not configured"
      )
    })
    ...
    it("does not allow decreasing the lock duration", async () => {
      await expectRevert(
        locker.increaseLockForDays(nftA.target, TOKEN_ID, 5),
        "invalid"
      )
    })

    it("allows increasing the lock duration", async () => {
      await locker.increaseLockForDays(nftA.target, TOKEN_ID, 15)
